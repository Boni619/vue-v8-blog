...
Published 12 November 2018 · Tagged with [ECMAScript](/blog/tags/ecmascript) [benchmarks](/blog/tags/benchmarks) [presentations](/blog/tags/presentations)
...
![](/_img/fast-async/await-bug-node-8.svg)
...
![](/_img/fast-async/await-bug-node-10.svg)
...
![](/_img/fast-async/microtasks-vs-tasks.svg)
...
When called it returns a promise, and you can get to its value like with any other promise.
    ...
    const p = computeAnswer();// → Promisep.then(console.log);// prints 42 on the next turn
...
Normally you’d pass a `Promise` to `await`, but you can actually wait on any arbitrary JavaScript value. If the value of the expression following the `await` is not a promise, it’s converted to a promise. That means you can `await 42` if you feel like doing that:
    ...
    async function foo() {  const v = await 42;  return v;}const p = foo();// → Promisep.then(console.log);// prints `42` eventually
...
![](/_img/fast-async/await-under-the-hood.svg)
...
![](/_img/fast-async/await-overhead.svg)
...
![](/_img/fast-async/await-overhead-removed.svg)
...
![](/_img/fast-async/node-10-vs-node-12.svg)
